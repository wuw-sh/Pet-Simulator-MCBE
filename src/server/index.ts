...
const $PetList: Record<string, string> = {
    Cat: 'minecraft:cat',
...
function getAllPet($player: $server.Player) {
    return Object.keys($PetList).map($pet => {
        const $petData = JSON.parse($server.world.getDynamicProperty(`pet:${$pet}`) as string)
        if (!$petData) $server.world.setDynamicProperty(`pet:${$pet}`, JSON.stringify({}))
        return Object.keys($petData).map($data => {
            if ($petData[$data] !== $player.name) return
...
$server.system.runInterval(() => {
    for (const $player of $server.world.getPlayers()) {
        const $pets = getAllPet($player).map((_, $index, $arr) => $index % 3 === 0 ? $arr.slice($index, $index + 3) : 0).filter(Boolean) as $server.Entity[][]
        ...
        .show($player).then($res => {
            ...
            switch ($res.selection) {
                ...
                case 1: {
                    ...
                    if ($pets.length === 0) return $player.sendMessage('§cYou have no pets')
                    ...
                    $removePets.show($player).then($res => {
                        ...
                        const $petData = JSON.parse($server.world.getDynamicProperty(`pet:${petDataName}`) as string)
